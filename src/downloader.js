...
async function downloadCSV() {
  logger.info('Iniciando download do CSV da Caixa (SP)...');

  try {
    const response = await client.get(DOWNLOAD_URL);

    const csv = iconv.decode(Buffer.from(response.data), 'ISO-8859-1');
    logger.info(`CSV baixado com sucesso (${csv.length} caracteres).`);
    return csv;
  } catch (error) {
    logger.error(`Falha ao baixar CSV após todas as tentativas: ${error.message}`);
    return null;
  }
}
